...
export default class Crop {
  ...
  setCrop({x, y, width, height}) {
    ...
    this.zoom({width: this.imageWidth * factor})

    // Position the view around the requested crop's center. When the requested
    // zoom is clamped (e.g. minResolution is unreachable at this ratio) this
    // keeps the crop centered on the requested area instead of drifting. When
    // it is not clamped this is equivalent to panning to (x, y).
    const previewFactor = this.preview.width / this.imageWidth
    this.pan({
      x: (x + width / 2) * previewFactor - this.viewWidth / 2,
      y: (y + height / 2) * previewFactor - this.viewHeight / 2
    })
  }
